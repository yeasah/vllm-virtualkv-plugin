...
class WorkerPager:
    ...
    def _tier_summary(self) -> dict[str, Any] | None:
        if not self.tier_rows:
            return None
        n = len(self.tier_rows)
        keys = set(self.tier_rows[0])
        for r in self.tier_rows:
            keys &= set(r)
        return {"steps": n,
                **{k: sum(r[k] for r in self.tier_rows) / n for k in keys}}
